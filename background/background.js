...
const chatState = {
  ...
  messages: [],
  tabId: null,
  contextUrl: ""   // URL whose page content is baked into messages[0]; refresh when it changes
};

// Voice Mode conversation — kept SEPARATE from chatState so the flirty voice
// persona + memory never affect typed chat. In-memory only (resets on reload).
const voiceChatState = {
  messages: [],        // [{role, content}] — capped at 40 (~20 turns), incl. agent-action notes
  pageUrl: "",
  pageSummaryText: ""
};
const VOICE_MEM_CAP = 40;

function pushVoiceMemory(entry) {
  voiceChatState.messages.push(entry);
  if (voiceChatState.messages.length > VOICE_MEM_CAP) {
    voiceChatState.messages = voiceChatState.messages.slice(-VOICE_MEM_CAP);
  }
}

function buildVoicePrompt(pageSummaryText, pageUrl) {
  let ctx = "";
  if (pageSummaryText && pageSummaryText.trim().length > 0) {
    ctx = "\n\nThe user is currently looking at this web page (use it only if they ask about it):\n" +
      "Page: " + (pageUrl || "(unknown)") + "\n" +
      pageSummaryText.slice(0, 2500);
  }
  return (
    "You are the user's warm, playful, companion, and they are TALKING to you OUT LOUD — " +
    "you will be HEARD, not read. Be charming, upbeat and clearly enjoying the chat, " +
    ...
    "- If the user asks you to DO something in the browser, don't worry — that's handled separately, " +
    "so just react warmly (you don't need to say you can't).\n" +
    "- You may be reminded of browser actions you already did for them; reference those naturally if relevant." +
    ctx
  );
}
...
const DEFAULT_CONFIG = {
  ...
  llmTimeout: 100000,
  wallTimeout: 300000,
  sttEngine: "chrome",   // "chrome" (browser Web Speech) | "groq" (Whisper API)
  groqApiKey: ""         // used only when sttEngine === "groq" (or auto-fallback in Brave)
};
...
async function loadConfig() {
  try {
    ...
    const cfg = {
      ...
      sttEngine: stored.sttEngine || DEFAULT_CONFIG.sttEngine,
      groqApiKey: stored.groqApiKey || DEFAULT_CONFIG.groqApiKey
    };
    if (stored.providers) {
...
- IMPORTANT: This extension can also act as an Agent that interacts with the browser (click buttons, fill forms, navigate, search, book, add to cart, check email, like a video, etc.). If the user asks you to DO something that requires browser interaction, do NOT simulate or describe the steps in chat. Instead reply with ONE short sentence telling them you'll handle it, and you MUST include the exact phrase "switching to Agent mode" — for example: "That needs me to act in the browser — switching to Agent mode to do it for you now." The extension automatically detects that phrase and runs the task as an agent on the current tab. Do not mention buttons or keyboard shortcuts.
...
/**
 * Heuristic tab decision for an agent task: should it run on the CURRENT tab
 * or open a NEW one? Instant, no LLM. Used for keyword-confident agent tasks
 * and as the fallback for the LLM classifier.
 *  - "current": the task is about the page the user is already on
 *  - "new": the task targets a different destination (don't disrupt current page)
 */
function decideTabHeuristic(text, url) {
  const t = (text || "").toLowerCase();
  if (/\bnew tab\b|\bin a new tab\b|\banother tab\b/.test(t)) return "new";
  // Explicit references to the current page → current tab
  if (/\bhere\b|\bon this\b|\bthis (page|tab|form|site|article|video|post|screen)\b|\bcurrent (page|tab|site)\b|\bfill (this|out|the)\b/.test(t)) return "current";
  // Navigation / search-a-site verbs → new tab
  if (/\b(open|go to|goto|navigate to|visit|launch)\b/.test(t)) return "new";
  if (/\bsearch\b.*\b(on|in)\b/.test(t)) return "new";
  // If the current tab is blank / new-tab / a bare search homepage, just use it
  if (!url || /^(about:blank|chrome:\/\/newtab|edge:\/\/newtab|brave:\/\/newtab)/.test(url)) return "current";
  try {
    const u = new URL(url);
    const h = u.hostname.replace(/^www\./, "");
    if (/^google\.[a-z.]+$/.test(h) && (u.pathname === "/" || u.pathname === "")) return "current";
  } catch (e) {}
  // Default: open a new tab so the user's current page isn't disrupted
  return "new";
}

/**
 * Combined classifier: decides intent (chat vs agent) AND, for agent tasks,
 * which tab to use — considering the message, recent conversation, and the
 * current tab. Returns { intent, tab }. One LLM call; JSON response.
 */
async function classifyMessageViaLLM(text, ctx) {
  await loadConfig();
  const provider = agentState.provider;
  const endpoint = agentState.endpoint.replace(/\/$/, "");
  const apiKey = agentState.apiKey;
  ...
  // Recent conversation for reference resolution (last couple of turns).
  let recent = "";
  try {
    const msgs = (chatState.messages || []).filter(m => m.role !== "system").slice(-4);
    if (msgs.length) {
      recent = "\nRecent conversation:\n" + msgs.map(m => (m.role === "user" ? "User: " : "Assistant: ") + (m.content || "").slice(0, 160)).join("\n");
    }
  } catch (e) {}

  const sysPrompt =
    'You classify a user message for a browser assistant. Decide two things and reply with ONLY a JSON object.\n' +
    '1. "intent": "chat" if the message is a question, conversation, or asks for information (about the current page or general knowledge); "agent" if it asks to perform a browser action (navigate, click, type, fill a form, search a site, book, buy, etc.).\n' +
    '2. "tab": only meaningful when intent is "agent". "current" if the task is about the page the user is currently on (e.g. "fill this form", "click login here", "summarize and reply on this page"). "new" if the task targets a different site/destination (e.g. "open YouTube", "search Amazon") so the user\'s current page is not disrupted.\n' +
    'If unsure about intent, choose "chat". If unsure about tab, choose "new".\n' +
    'Reply with ONLY: {"intent":"chat"|"agent","tab":"current"|"new"}';

  const userPrompt =
    'Current tab URL: ' + (ctx.tabUrl || "(none)") + '\n' +
    'Current tab title: ' + (ctx.tabTitle || "(none)") + recent + '\n\n' +
    'User message: "' + text + '"\n\nClassify now as JSON.';
  ...
  if (fmt === "ollama") {
    ...
    headers = { "Content-Type": "application/json", "Authorization": "Bearer " + apiKey };
    body = JSON.stringify({
      model, temperature: 0, max_tokens: 40, response_format: { type: "json_object" },
      messages: [{ role: "system", content: sysPrompt }, { role: "user", content: userPrompt }]
    });
  } else if (fmt === "claude") {
    url = endpoint + "/v1/messages";
    headers = { "Content-Type": "application/json", "x-api-key": apiKey, "anthropic-version": "2023-06-01" };
    body = JSON.stringify({
      model, system: sysPrompt, max_tokens: 40, temperature: 0,
    ...
    return { intent: "chat", tab: decideTabHeuristic(text, ctx.tabUrl) };
  }
  ...
  try {
    ...
    const resp = await fetch(url, { method: "POST", headers, body, signal: controller.signal });
    clearTimeout(timerId);
    if (!resp.ok) return { intent: "chat", tab: decideTabHeuristic(text, ctx.tabUrl) };
    const raw = await resp.json();

    let content = "";
    if (fmt === "ollama") content = (raw.message && raw.message.content) || "";
    else if (fmt === "openai") content = (raw.choices && raw.choices[0] && raw.choices[0].message && raw.choices[0].message.content) || "";
    else if (fmt === "claude") content = (raw.content && raw.content[0] && raw.content[0].text) || "";
    else if (fmt === "gemini") content = (raw.candidates && raw.candidates[0] && raw.candidates[0].content && raw.candidates[0].content.parts && raw.candidates[0].content.parts[0] && raw.candidates[0].content.parts[0].text) || "";
    ...
    const intent = (parsed && parsed.intent === "agent") ? "agent" : "chat";
    let tab = (parsed && (parsed.tab === "current" || parsed.tab === "new")) ? parsed.tab : decideTabHeuristic(text, ctx.tabUrl);
    return { intent, tab };
  } catch {
    return { intent: "chat", tab: decideTabHeuristic(text, ctx.tabUrl) };
  }
...
// Capture the current page and build the chat system prompt for a tab.
// Returns { systemPrompt, summary, tabUrl, pageTitle, pageUrl, preview }.
// `summary` is null on Chrome-restricted pages (new tab, chrome://, etc.).
// Throws if a normal page's content can't be captured.
async function buildChatContextForTab(tabId) {
  let tabUrl = "";
  try { tabUrl = (await chrome.tabs.get(tabId)).url || ""; } catch {}

  const historyQueue = await loadHistoryQueue();

  // Chrome-restricted page — chat works without page context.
  if (isBlockedUrl(tabUrl)) {
    const systemPrompt = `You are a helpful AI assistant embedded in a browser extension. The user is on a browser internal page (like a new tab) with no web content to analyse — just answer as a general-purpose assistant.\n\nRULES:\n- Be conversational, direct, and helpful.\n- You may use **bold** and *italic* for emphasis, and bullet points (- item) for short lists.\n- This extension also has an "Agent mode" that can interact with websites. If the user asks you to DO something on the web, suggest they navigate to a page first and use Agent mode.` +
      (historyQueue && historyQueue.length > 0
        ...
        : "");
    return { systemPrompt, summary: null, tabUrl, pageTitle: "New Tab", pageUrl: tabUrl, preview: "" };
  }
...
async function initChat(tabId) {
  ...
  try {
    const ctx = await buildChatContextForTab(tabId);
    chatState.pageSummary = ctx.summary;
    chatState.contextUrl = ctx.tabUrl;
    chatState.messages = [{ role: "system", content: ctx.systemPrompt }];
    return { success: true, pageTitle: ctx.pageTitle, pageUrl: ctx.pageUrl, preview: ctx.preview };
  } catch (err) {
...
// Refresh the baked-in page context when the active tab has navigated or the
// user switched to a different tab — WITHOUT wiping the conversation. Only the
// system prompt (messages[0]) is rebuilt; all user/assistant turns are kept.
async function refreshChatContextIfChanged(tabId) {
  if (!chatState.active || tabId == null) return;

  let url = "";
  try { url = (await chrome.tabs.get(tabId)).url || ""; } catch { return; }
  if (!url || url === chatState.contextUrl) return; // same page → nothing to do
  ...
  try {
    ...
    chatState.contextUrl = ctx.tabUrl;
    chatState.tabId = tabId;
    const sysMsg = { role: "system", content: ctx.systemPrompt };
    if (chatState.messages.length > 0 && chatState.messages[0].role === "system") {
      chatState.messages[0] = sysMsg;          // swap context, keep the conversation
    } else {
      chatState.messages.unshift(sysMsg);
    }
  } catch (err) {
    // Page still loading or not capturable — keep the previous context.
    console.warn("[Chat] context refresh skipped:", err.message);
  }
}
...
chrome.runtime.onMessage.addListener((msg, sender, sendResponse) => {
  switch (msg.type) {
      ...
      (async () => {
        try {
          if (!chatState.active) {
            ...
            // Already chatting — refresh page context if the user navigated this
            // tab or switched to a different one (keeps the conversation intact).
            await refreshChatContextIfChanged(tabId);
          }
    ...
    // ── Voice Chat (flirty companion; SEPARATE from typed chat) ──
    case "VOICE_CHAT": {
      const text = msg.text;
      const tabId = msg.tabId;
      if (!text) { sendResponse({ success: false, error: "Empty message." }); return true; }
      ...
      (async () => {
        try {
          ...
          // Best-effort page context — only re-capture if the tab changed.
          if (tabId != null) {
            try {
              const tab = await chrome.tabs.get(tabId);
              const url = (tab && tab.url) || "";
              if (url && url !== voiceChatState.pageUrl && !isBlockedUrl(url)) {
                const summary = await capturePageSummary(tabId);
                voiceChatState.pageUrl = url;
                voiceChatState.pageSummaryText = (summary && summary.text) || "";
              }
            } catch (e) { /* keep whatever context we had */ }
          }

          const sys = buildVoicePrompt(voiceChatState.pageSummaryText, voiceChatState.pageUrl);
          const msgs = [{ role: "system", content: sys }].concat(voiceChatState.messages, [{ role: "user", content: text }]);

          const chatModel = agentState.chatModel || agentState.model;
          const result = await callLLM(msgs, chatModel, "VoiceChat", null, { forceJson: false });
          const reply = (result && result._rawContent) || "";

          pushVoiceMemory({ role: "user", content: text });
          pushVoiceMemory({ role: "assistant", content: reply });
    ...
    // ── Voice memory: record an agent action the companion performed ──
    case "VOICE_REMEMBER": {
      const goal = (msg.goal || "").toString().slice(0, 200);
      const summary = (msg.summary || "").toString().slice(0, 400);
      if (goal || summary) {
        pushVoiceMemory({
          role: "system",
          content: '[You just did this for the user via the browser: "' + goal + '" — ' + summary + "]"
        });
      }
      sendResponse({ success: true });
      return true;
    ...
    // ── Voice memory reset (voice mode turned off) ──
    case "VOICE_RESET": {
      voiceChatState.messages = [];
      voiceChatState.pageUrl = "";
      voiceChatState.pageSummaryText = "";
      sendResponse({ success: true });
      return true;
    }

    // ── Agent Send (explicit agent mode) ──
    // The agent always operates on the CURRENT tab — it never opens a new one.
    // ensureInjectableTab (inside runAgentLoop) navigates the current tab to a
    // usable page if it's on a non-injectable URL (chrome://, etc.).
    case "AGENT_SEND": {
      const text = msg.text;
      if (!text) { sendResponse({ success: false, error: "Empty message." }); return true; }
      ...
      (async () => {
        try {
          ...
          let tabId = msg.tabId;
          // No tab id supplied — fall back to the active tab (never create one).
          if (!tabId) {
            try {
          ...
          if (!tabId) { sendResponse({ success: false, error: "No active tab to run on." }); return; }
    ...
    // Fast keyword classifier first; if it's not confident, fall back to the
    // LLM classifier (which defaults to "chat" when unsure). This matters for
    // Voice Mode: spoken questions like "what's on this page" have no action
    // keyword and must route to chat, not default to agent.
    case "CLASSIFY_INTENT": {
      const fast = classifyIntent(msg.text || "");
      if (fast.confident) {
        sendResponse({ intent: fast.intent });
        return true;
      }
      (async () => {
        try {
          const intent = await classifyIntentViaLLM(msg.text || "");
          sendResponse({ intent });
        } catch (err) {
          sendResponse({ intent: fast.intent });
        }
    ...
    // ── Combined classification: intent (chat vs agent) + tab (current vs new) ──
    // Used by typed input and Voice Mode. Fast keyword classifier handles clear
    // action commands instantly (with a heuristic tab pick); ambiguous messages
    // get one LLM call that decides both intent and tab from message + context.
    case "CLASSIFY_MESSAGE": {
      const text = msg.text || "";
      const tabUrl = msg.tabUrl || "";
      const tabTitle = msg.tabTitle || "";
      const fastC = classifyIntent(text);
      if (fastC.confident) {
        sendResponse({ intent: "agent", tab: decideTabHeuristic(text, tabUrl) });
        return true;
      }
      (async () => {
        try {
          const r = await classifyMessageViaLLM(text, { tabUrl, tabTitle });
          sendResponse({ intent: r.intent || "chat", tab: r.tab || decideTabHeuristic(text, tabUrl) });
        } catch (err) {
          sendResponse({ intent: "chat", tab: decideTabHeuristic(text, tabUrl) });
        }
      })();
      return true; // async sendResponse
    }
